---
title: 搭建 Vue3 组件库次佳实践-编码规范与提交规范
date: 2022-08-13 15:38:03
updated: 2022-08-13 15:38:03
categories: Tools
---

> 如果你还没有阅读 [搭建 Vue3 组件库次佳实践-开篇](./2022-08-10.md)，那么可以先阅读此篇中对整体系列的概述，以便你更好的理解整个系列的全貌。

这篇我们搭建一个可共享的规范配置。所谓可共享就是把我们常用的配置提取成一个单独的包利用配置继承的特性可以在不同的项目中使用。就算我们不单独发布包也可以拆分出来，便于管理结构和依赖。

接下来我们就就把社区中常用的规范工具集成到项目中，本篇新增的完整代码可查看单独的分支 [1-lint](https://github.com/bfehub/vlib-starter/tree/1-lint)。

也顺便推荐我之前写的编码规范、提交规范、项目配置文件脚手架工具 [lint](https://github.com/bfehub/lint)，比这篇功能完善点，也可参考学习。
...
我们先基于我们基础结构在 `packages` 创建一个 `eslint-config` 子文件夹。包名称为 `@vlib/eslint-config`，并安装相关依赖。

```json
// packages/eslint-config/package.json
{
  "name": "@vlib/eslint-config",
  "main": "index.js",
  "version": "1.0.0",
...
```bash
pnpm add eslint -D -w
pnpm add eslint -D --filter @vlib/eslint-config
```
...
pnpm add prettier -D --filter @vlib/eslint-config
```
...
pnpm add typescript -D --filter @vlib/eslint-config
# 解析器和内置规则集
...
```sh
pnpm add eslint-plugin-vue --filter @vlib/eslint-config
```
...
我们接着在 `packages` 创建一个 `stylelint-config` 子文件夹。包名称为 `@vlib/stylelint-config`，并安装相关依赖。

```json
// packages/stylelint-config/package.json
{
  "name": "@vlib/stylelint-config",
  "main": "index.js",
  "version": "1.0.0",
  "description": "Eslint Config."
}
...
```sh
pnpm add @vlib/stylelint-config -D -w
```
...
```sh
pnpm add prettier -D -w
pnpm add prettier -D --filter @vlib/stylelint-config
```
...
```sh
pnpm add stylelint-prettier stylelint-config-prettier --filter @vlib/stylelint-config
```
