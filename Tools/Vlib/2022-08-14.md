---
title: 搭建 Vue3 组件库次佳实践-组件演示文档
date: 2022-08-14 11:15:20
updated: 2022-08-14 11:15:20
categories: Tools
---

> 如果你还没有阅读 [搭建 Vue3 组件库次佳实践-一个完整的系列指南](./2022-08-10.md)，那么可以先阅读此篇中对整体系列的概述，以便你更好的理解整个系列的全貌。

这篇我们来搭建一个文档系统，文档系统有很多选择比如 storybook、vuepress、vitepress 或者自建文档。这里选择基于 vuepress 搭建，因为 vuepress 功能完善也容易扩展。

接下来我们就基于 vuepress 搭建文件，并自己实现一个组件演示的插件。本篇新增的完整代码可查看单独的分支 [2-docs](https://github.com/bfehub/vlib-starter/tree/2-docs)。

也顺便推荐我之前写的用于组件开发场景的 VuePress 的插件和主题 [vmi](https://github.com/bfehub/vmi)，实现了接近于 dumi 的体验和功能。
...
```sh
# 前置依赖和工具包
pnpm add vue rimraf anywhere -D --filter docs
# VuePress 依赖
pnpm add vuepress@next @vuepress/client@next  -D --filter docs
```
...
默认情况下 `VuePress` 只抓取源目录的文件(`docs`)，但是我们设计的文档和组件是放在一起的在 `packages/ui/src` 下各个组件内，这时我们改怎么处理？
...
<!-- packages/ui/src/button/README.md -->

# button
```

这里我们启动文档服务即可访问到我们在 `packages/ui/src/button/README.md` 中写的文档。
...
```html
<!-- packages/ui/src/button/__demos__/basic.vue -->
<template>
  <div>
    <button>按钮一</button>
    <button>按钮二</button>
...
我们继续在 `packages` 创建一个 `vuepress-plugins` 子文件夹。包名称为 `@vlib/vuepress-plugins`，并安装相关依赖。

```json
// packages/vuepress-plugins/package.json
{
  "name": "@vlib/vuepress-plugins",
  "main": "index.ts",
  "version": "1.0.0",
...
# 核心开发工具包
pnpm add @vuepress/core@next @vuepress/client@next @vuepress/utils@next --filter @vlib/vuepress-plugins

# 解析 markdown 语法工具包
pnpm add @vuepress/markdown@next @vuepress/plugin-prismjs@next --filter @vlib/vuepress-plugins
...
```sh
pnpm add @vlib/vuepress-plugins -D --filter docs
```
...
2. 使用 posthtml-parser 解析 `demo` 的标签 `src` 属性。
