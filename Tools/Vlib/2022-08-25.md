---
title: "实践 Vue3 组件库-如何实现组件库级别的打包流程"
...
提取外部依赖，根据 `package.json` 中的 `dependencies` 和 `peerDependencies` 来自动生成外部列表来是否是外部依赖。
...
使用 [create vite](https://github.com/vitejs/vite/tree/main/packages/create-vite) 创建一个示例项目测试各种模块的使用和效果(注：现在还没有打包样式但是不影响效果)。
