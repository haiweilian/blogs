...
最近在开发移动端的项目，所以此篇就基于 vant 开发一个组件，本篇新增的完整代码可查看单独的分支 [3-ui](https://github.com/bfehub/vlib-starter/tree/3-ui)。
...
```ts
// packages/ui/src/utils/create.ts
...
```ts
// packages/ui/src/back-top/src/props.ts
...
```ts
// packages/ui/src/back-top/index.ts
...
```scss
// packages/ui/src/back-top/index.scss
body {
...
在 `setup` 中指定组件名称必须再多写个 `script` 标签，但并不想这么做。你可能注意到上面的 `defineOptions` 这个编译宏并不属于 `vue` 内置的，它是由 [unplugin-vue-macros](https://github.com/sxzz/unplugin-vue-macros) 这个插件提供的。

先解决 `defineOptions` 在代码中提示找不到的问题，安装此依赖并在 `tsconfig.json` 中加入类型声明。

```sh
pnpm add unplugin-vue-macros -D -w
```
...
```sh
pnpm add unplugin-vue-macros -D --filter docs
```
...
```sh
pnpm add vant @vlib/ui --filter docs
```
...
```vue
<!-- packages/ui/src/back-top/__demos__/basic.vue -->
<template>
  <vlib-back-top />
</template>
```
...
<!-- packages/ui/src/back-top/README.md -->
